...
import { StyleSheet, View, ScrollView, Text, Alert, TouchableOpacity, AsyncStorage, Modal, Pressable,TextInput } from 'react-native';
...
const updatecloplomapping = ({ navigation }) => {
  ...
  const AddClosActivityMappingAction= async (list) => {
    ...
    try {
      ...
      const apiUrl = `${global.apiURL}/CLOsPLOsMapping/CLOsMappingPLOs`;
  ...
  const handleWeightageChange = (ploId, cloId, id, weightage) => {
        ...
        if (totalWeightage > 100) {
         Alert.alert(`Total weightage for CLO ${cloId} exceeds 100`);
          ...
          if (!existingWeight) {
            setWeights((weights) => [
              ...weights,
              { Plo_Id: ploId, Clo_Id: cloId, Weightage: Number(latestWeightage )},
            ]);
          } else {
            setWeights((weights) =>
              weights.map((w) =>
  ...
  useEffect(() => {
    const uniquePlos = cloactivityData.reduce((acc, curr) => {
      const existingPair = acc.find(
        (pair) => pair.Name === curr.Name && pair.Plo_Id === curr.Plo_Id
      );
      if (!existingPair) {
        acc.push({
          Plo_Id: curr.Plo_Id,
    ...
    const uniqueClos = cloactivityData.reduce((acc, curr) => {
      ...
      if (!existingPair) {
        acc.push({
          Clo_Id: curr.Clo_Id,
          Name: curr.cloname,
          Description: curr.cloDescription,
        });
      }
      return acc;
    }, []);
    ...
    if (result == true) {
      ...
      const updatedArray = cloactivityData?.map((item1) => {
        const item2 = updateWeights.find(
          (item2) => item1.Clo_Id === item2.Clo_Id && item1.Plo_Id  === item2.Plo_Id 
        );
        if (item2) {
          return {
            ...item1,
  ...
  return (
    <View style={styles.container}>
        ...
        <View>
          <Table borderStyle={{ borderWidth: 1, borderColor: 'black' }}>
                        ...
                        <Row
                             ...
                             data={['C/P', 'total', ...plosName.map((plos) => plos.Name)]}
                             widthArr={[55,55,  ...Array(plosName.length).fill(45)]}
                            style={styles.head}
     ...
     <Text style={styles.text1 }> {clos.Name}</Text>  
        ...
        `${
          getColumnRemaingTotal(clos.Clo_Id) === null
            ? '0%'
            : getColumnRemaingTotal(clos.Clo_Id)
        }%`
                ,
                  ...plosName.map((plos) => {
                    ...
                    return (
                      ...
                      key={`${clos.Clo_Id}-${plos.Plo_Id}`}
                        ...
                        onBlur={() => {
                          handleWeightageChange(plos.Plo_Id, clos.Clo_Id, weight.Id,  inputText);
                        }}
                      />
...
const styles = StyleSheet.create({
  ...
  ROWTABLE: { height: 42,borderColor:'black'  },
...
export default updatecloplomapping